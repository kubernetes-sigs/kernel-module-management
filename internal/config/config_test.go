...
var _ = Describe("MergeUserConfigInto", func() {
	...
	It("should return nil if uc is nil", func() {
		...
		Expect(config.Job.GCDelay).To(Equal(1 * time.Minute))
	})

	It("should apply user config to Config obj", func() {
		uc := &UserConfig{
			SELinuxType:      ptr.To("foo_t"),
			FirmwareHostPath: ptr.To("/bar"),
			GCDelay:          ptr.To(1 * time.Minute),
		}
		cfg := &Config{}
		...
		Expect(cfg.Job.GCDelay).To(Equal(1 * time.Minute))
	})
})
