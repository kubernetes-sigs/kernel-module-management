...
func (r *ManagedClusterModuleReconciler) Reconcile(ctx context.Context, mcm *hubv1beta1.ManagedClusterModule) (ctrl.Result, error) {
	...
	for _, cluster := range clusters.Items {
		...
		allImagesReady, err := r.reconHelper.handleMIC(ctx, mcm, cluster.Name, kernelVersions)
		if err != nil {
			logger.Info(utils.WarnString(
				fmt.Sprintf("Failed to handle MIC: %v", err),
			))
			continue
		}
		if !allImagesReady {
...
type managedClusterModuleReconcilerHelperAPI interface {
	handleMIC(ctx context.Context, mcm *hubv1beta1.ManagedClusterModule, clusterName string, kernelVersions []string) (bool, error)
}
...
func (rh *managedClusterModuleReconcilerHelper) handleMIC(ctx context.Context, mcm *hubv1beta1.ManagedClusterModule,
	clusterName string, kernelVersions []string) (bool, error) {
	...
	for _, kver := range kernelVersions {
		...
		if err != nil {
			if !errors.Is(err, module.ErrNoMatchingKernelMapping) {
				errs = append(errs, fmt.Errorf("failed to get MLD for kernel %s: %v", kver, err))
			}
