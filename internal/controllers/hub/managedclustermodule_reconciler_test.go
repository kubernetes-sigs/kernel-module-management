...
var _ = Describe("ManagedClusterModuleReconciler_Reconcile", func() {
	...
	It("should skip failed to handle MICs", func() {
		...
		gomock.InOrder(
			...
			mockMCMReconHelperAPI.EXPECT().handleMIC(ctx, mcm, "cluster-1", expectedKernelVersion).Return(false, errors.New("error")),
	...
	It("should not create manifestWork for MIC that doesn't have all images in the exist state", func() {
		...
		gomock.InOrder(
			mockClusterAPI.EXPECT().SelectedManagedClusters(ctx, mcm).Return(expectedClusters, nil),
			mockClusterAPI.EXPECT().KernelVersions(expectedClusters.Items[0]).Return(expectedKernelVersion, nil),
			mockMCMReconHelperAPI.EXPECT().handleMIC(ctx, mcm, "cluster-1", expectedKernelVersion).Return(false, nil),
			// we expecte the rest of the loop to be skipped with no errors since not all images are ready
			mockManifestAPI.EXPECT().GarbageCollect(ctx, *expectedClusters, *mcm).Return(nil),
			mockManifestAPI.EXPECT().GetOwnedManifestWorks(ctx, *mcm).Return(expectedOwnManifestWork, nil),
			mockStatusupdaterAPI.EXPECT().ManagedClusterModuleUpdateStatus(ctx, mcm, expectedOwnManifestWork.Items).Return(nil),
		)
	...
	It("should work as expected", func() {
		...
		mockMCMReconHelperAPI.EXPECT().handleMIC(ctx, mcm, "cluster-1", expectedKernelVersion).Return(true, nil)
		mockMCMReconHelperAPI.EXPECT().handleMIC(ctx, mcm, "cluster-2", expectedKernelVersion).Return(true, nil)
...
var _ = Describe("managedClusterModuleReconcilerHelperAPI_handleMIC", func() {

	const (
		mcmName      = "mcm-name"
		mcmNamespace = "mcm-namespace"
	...
	It("should return an error if we faild to create or patch the MIC", func() {
		...
		gomock.InOrder(
			...
			mockMIC.EXPECT().CreateOrPatch(ctx, mcmName+"-"+clusterName, mcmNamespace, gomock.Any(), nil, mcm).
				Return(errors.New("some error")),
		)

		_, err := mcmReconHelperAPI.handleMIC(ctx, mcm, clusterName, kernelVersions)
		Expect(err).To(HaveOccurred())
	...
	It("should return an error if we faild to get the MIC", func() {
		...
		gomock.InOrder(
			mockClusterAPI.EXPECT().GetModuleLoaderDataForKernel(mcm, kernelVersions[0]).Return(&api.ModuleLoaderData{}, nil),
			mockMIC.EXPECT().CreateOrPatch(ctx, mcmName+"-"+clusterName, mcmNamespace, gomock.Any(), nil, mcm).Return(nil),
			mockMIC.EXPECT().Get(ctx, mcmName, mcmNamespace).Return(nil, errors.New("some error")),
		)
	...
	It("should skip non targeted kernel versions", func() {
		...
		gomock.InOrder(
			...
			mockMIC.EXPECT().CreateOrPatch(ctx, mcmName+"-"+clusterName, mcmNamespace, expectedImages, gomock.Any(), mcm).Return(nil),
			mockMIC.EXPECT().Get(ctx, mcmName, mcmNamespace).Return(expectedMIC, nil),
