...
func (mr *ModuleReconciler) Reconcile(ctx context.Context, mod *kmmv1beta1.Module) (ctrl.Result, error) {
	...
	if err := mr.micAPI.ApplyMIC(ctx, mod.Name, mod.Namespace, images, mod.Spec.ImageRepoSecret, mod); err != nil {

		return ctrl.Result{}, fmt.Errorf("failed to apply %s/%s MIC: %v", mod.Namespace, mod.Name, err)
	}
