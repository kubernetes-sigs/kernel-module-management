...
func (r *mbscReconciler) Reconcile(ctx context.Context, mbscObj *kmmv1beta1.ModuleBuildSignConfig) (ctrl.Result, error) {
	...
	err := r.reconHelperAPI.updateStatusByBuilds(ctx, mbscObj)
	if err != nil {
...
type mbscReconcilerHelperAPI interface {
	updateStatusByBuilds(ctx context.Context, mbscObj *kmmv1beta1.ModuleBuildSignConfig) error
...
func (mrh *mbscReconcilerHelper) updateStatusByBuilds(ctx context.Context, mbscObj *kmmv1beta1.ModuleBuildSignConfig) error {
	...
	for _, imageSpec := range mbscObj.Spec.Images {
		status, err := mrh.buildSignAPI.GetStatus(ctx, mbscObj.Name, mbscObj.Namespace, imageSpec.ModuleImageSpec.KernelVersion, imageSpec.Action, mbscObj)
...
func (mrh *mbscReconcilerHelper) processImagesSpecs(ctx context.Context, mbscObj *kmmv1beta1.ModuleBuildSignConfig) error {
	...
	for _, imageSpec := range mbscObj.Spec.Images {
		...
		err := mrh.buildSignAPI.Sync(ctx, mld, true, mbscObj, imageSpec.Action)
		if err != nil {
			errs = append(errs, err)
