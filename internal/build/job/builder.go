...
func NewBuilder(helper build.Helper, scheme *runtime.Scheme) Maker {
...
func (m *builder) ShouldRun(mod *kmmv1beta1.Module, km *kmmv1beta1.KernelMapping) bool {
	// if theres no Dockerfile we have no way to build anything anyway

	if (mod.Spec.ModuleLoader.Container.Build == nil || mod.Spec.ModuleLoader.Container.Build.Dockerfile == "") &&
		(km.Build == nil || km.Build.Dockerfile == "") {
		return false
	}
	return true
}

func (m *builder) MakeJob(mod kmmv1beta1.Module, buildConfig *kmmv1beta1.Build, targetKernel, previousImage string, containerImage string, pushImage bool) (*batchv1.Job, error) {
	args := []string{}
	if pushImage {
