...
var _ = Describe("GetStatus", func() {
	var (
		...
		mockBuildSignPodManager *buildsignpod.MockBuildSignPodManager
	...
	BeforeEach(func() {
		...
		mockBuildSignPodManager = buildsignpod.NewMockBuildSignPodManager(ctrl)
	...
	It("failed flow, GetModulePodByKernel fails", func() {
		...
		mockBuildSignPodManager.EXPECT().GetModulePodByKernel(ctx, mbscName, mbscNamespace, normalizedKernel, buildsignpod.PodTypeBuild, &testMBSC).
	...
	It("GetModulePodByKernel returns pod does not exists", func() {
		normalizedKernel := kernel.NormalizeVersion(kernelVersion)
		mockBuildSignPodManager.EXPECT().GetModulePodByKernel(ctx, mbscName, mbscNamespace, normalizedKernel, buildsignpod.PodTypeBuild, &testMBSC).
			Return(nil, buildsignpod.ErrNoMatchingPod)
	...
	It("failed flow, GetPodStatus fails", func() {
		...
		gomock.InOrder(
			mockBuildSignPodManager.EXPECT().GetModulePodByKernel(ctx, mbscName, mbscNamespace, normalizedKernel, buildsignpod.PodTypeBuild, &testMBSC).
				Return(&foundPod, nil),
			mockBuildSignPodManager.EXPECT().GetPodStatus(&foundPod).Return(buildsignpod.Status(""), fmt.Errorf("some error")),
		)
	...
	DescribeTable("check good flow and returned statuses", func(podStatus buildsignpod.Status, expectedStatus kmmv1beta1.BuildOrSignStatus) {
		foundPod := v1.Pod{}
		normalizedKernel := kernel.NormalizeVersion(kernelVersion)
		gomock.InOrder(
			mockBuildSignPodManager.EXPECT().GetModulePodByKernel(ctx, mbscName, mbscNamespace, normalizedKernel, buildsignpod.PodTypeBuild, &testMBSC).
		...
		Entry("pod's status is success", buildsignpod.StatusCompleted, kmmv1beta1.ActionSuccess),
		Entry("pod's status is failure", buildsignpod.StatusFailed, kmmv1beta1.ActionFailure),
		Entry("pod's status is in progress", buildsignpod.StatusInProgress, kmmv1beta1.BuildOrSignStatus("")),
		Entry("pod's status is in unknown", buildsignpod.Status(""), kmmv1beta1.BuildOrSignStatus("")),
	)
})
...
var _ = Describe("Sync", func() {
	...
	It("GetModulePodByKernel failed", func() {
		gomock.InOrder(
			...
			mockBuildSignPodManager.EXPECT().GetModulePodByKernel(ctx, mbscName, mbscNamespace, kernelVersion, buildsignpod.PodTypeBuild, &testMBSC).
				Return(nil, fmt.Errorf("some error")),
		)
		err := mgr.Sync(ctx, testMLD, true, &testMBSC, kmmv1beta1.BuildImage)
		Expect(err).To(HaveOccurred())
	...
	DescribeTable("check good flow", func(buildAction, podExists, podChanged, pushImage bool) {
		...
		if !podExists {
			getPodError = buildsignpod.ErrNoMatchingPod
		}
...
var _ = Describe("GarbageCollect", func() {
	...
	It("failed to get module pods", func() {
		mockBuildSignPodManager.EXPECT().GetModulePods(ctx, mbscName, mbscNamespace, buildsignpod.PodTypeBuild, &testMBSC).Return(nil, fmt.Errorf("some error"))

		_, err := mgr.GarbageCollect(ctx, mbscName, mbscNamespace, buildsignpod.PodTypeBuild, &testMBSC)
		Expect(err).To(HaveOccurred())
	})
	...
	It("delete pod failed", func() {
		...
		gomock.InOrder(
			mockBuildSignPodManager.EXPECT().GetModulePods(ctx, mbscName, mbscNamespace, buildsignpod.PodTypeBuild, &testMBSC).
	...
	It("good flow", func() {
		...
		res, err := mgr.GarbageCollect(ctx, mbscName, mbscNamespace, buildsignpod.PodTypeBuild, &testMBSC)
		Expect(err).To(BeNil())
