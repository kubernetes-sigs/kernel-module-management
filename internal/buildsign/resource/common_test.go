...
COPY --from=source /modules /modules
RUN for file in /modules/simple-kmod.ko; do \
      [ -e "${file}" ] && /usr/local/bin/sign-file sha256 /run/secrets/key/key.pem /run/secrets/cert/cert.pem "${file}" || true; \
    done
RUN for file in /modules/simple-procfs-kmod.ko; do \
      [ -e "${file}" ] && /usr/local/bin/sign-file sha256 /run/secrets/key/key.pem /run/secrets/cert/cert.pem "${file}" || true; \
    done

FROM source
COPY --from=signimage /modules /modules
`
	)
	...
	DescribeTable("should generate correct Dockerfile for signing",
		...
		Entry(
			...
			[]string{
				"COPY --from=source /modules /modules",
				"for file in /modules/test.ko; do",
		...
		Entry(
			...
			[]string{
				"COPY --from=source /modules /modules",
				"for file in /modules/a.ko; do",
				"for file in /modules/b.ko; do",
			},
