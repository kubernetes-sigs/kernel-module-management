...
func main() {
	...
	if ctrlConfig.Spec.Reconcilers.ManagedClusterModule.Enabled {
	...
	if modConfig := ctrlConfig.Spec.Reconcilers.Module; modConfig.Enabled {
		...
		if modConfig.AllowBuilds {
			b = buildAPI
		}

		if modConfig.AllowSigning {
			s = signAPI
		}
	...
	if ctrlConfig.Spec.Reconcilers.NodeKernelClusterClaim.Enabled {
	...
	if ctrlConfig.Spec.Reconcilers.NodeKernel.Enabled {
	...
	if ctrlConfig.Spec.Reconcilers.PodNodeModule.Enabled {
	...
	if ctrlConfig.Spec.Reconcilers.PreflightValidation.Enabled {
